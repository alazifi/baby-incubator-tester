//ini merubah bentuk string pada penyimpanan microSD
//mencoba arahan dari pak Tito, namun masih saja membutuhkan 79% dinamic Memory
...
// String untuk data dan header
String dataString;
const String headerString = "Data, T1, T2, T3, T4, T5, T6, Humadity, Air Speed, Sound Level";
int sampleWindow = 50;
int sample;
File myFile;
...
// Fungsi untuk mengubah data menjadi string
void formatDataToString() {
  dataString = "";
  dataString += dataCount;
  dataString += ", ";
  dataString += temp10;
  dataString += ", ";
  dataString += temp20;
...
void WriteDataToSD(const char* data) {
  ...
  formatDataToString();
  // Buka file
  File myFile = SD.open("hasil1.txt", FILE_WRITE);

  // Tulis data jika file berhasil dibuka
  if (myFile) {
    Serial.println(F("Mencoba menulis data ke hasil1.txt"));
    if (dataCount == 1) {
      myFile.println();
